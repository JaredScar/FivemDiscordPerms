...
{}, -- Admin (7)
{"",}, -- Management (8)
{"WEAPON_RPG", "WEAPON_GRENADELAUNCHER", "WEAPON_STICKYBOMB", "WEAPON_GRENADE", "WEAPON_HOMINGLAUNCHER", "WEAPON_PROXMINE", "WEAPON_PIPEBOMB",
...
Citizen.CreateThread(function()
    while true do
		...
		local restrictedAttachment = false
		...
		if not has_value(isAllowed, requiredPerm) and requiredPerm ~= nil then
			...
			if restrictedAttachment then
				DisplayNotification("~r~Weapon has restricted attachment")
			else
				DisplayNotification("~r~Restricted Weapon")
			end
		end
